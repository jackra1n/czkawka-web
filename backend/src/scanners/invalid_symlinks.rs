...
pub fn run(
    ...
    let invalid_symlinks = finder.get_invalid_symlinks();

    let mut groups = Vec::new();
    let mut total_files = 0;

    for entry in invalid_symlinks {
        ...
        let similarity = Some(format!(
            "{} ({})",
            entry.symlink_info.destination_path.to_string_lossy(),
            entry.symlink_info.type_of_error
        ));
